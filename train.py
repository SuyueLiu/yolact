    ...
    def gather(self, outputs, output_device):
        ...
        for k in outputs[0]:
            ...
            for idx, output in enumerate(outputs):
                if k not in output:
                    print(idx, output.keys())
                temp.append(output[k].to(output_device))
            out[k] = torch.stack(temp)
...
def train():
    ...
    net = CustomDataParallel(NetLoss(net, criterion), device_ids=[0, 1, 2, 3, 4, 5, 6, 7])
    if args.cuda:
        net = net.cuda()
